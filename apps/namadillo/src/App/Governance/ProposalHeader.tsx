...
import { showProposalId } from "utils";
import {
  ...
  return (
    <>
      ...
      <div className="flex gap-10 bg-neutral-900 mb-9 px-5 py-3 -mx-3 rounded-md">
        <Progress proposal={proposal} cachedProposal={cachedProposal} />
...
const Progress: React.FC<{
  ...
  return (
    <div className="w-full grid grid-cols-2 text-xs">
      ...
      >
        <span>Progress</span>{" "}
    {proposal.status !== "pending" && propoal.status !== "error" && (
        <span>
...
const ProgressStartEnd: React.FC<{
  ...
  <div className={className}>
    {proposal.status === "pending" || proposal.status === "error" ?
      "..."
      : stringFromTimestampInSec(proposal.data[timeKey])}
  </div>
);
...
const ProgressBar: React.FC<{
  ...
  const { value, total } = (() => {
    ...
    const { startTime, endTime, currentTime } = proposal.data;
